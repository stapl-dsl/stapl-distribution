...
import stapl.distribution.components.ContinuousOverloadClientForConcurrentCoordinators
...
object ContinuousOverloadClientForConcurrentCoordinatorsApp {
  def main(args: Array[String]) {
    ...
    parser.parse(args, ContinuousOverloadClientForConcurrentCoordinatorConfig()) map { config =>
      ...
      val client1 = system.actorOf(Props(classOf[ContinuousOverloadClientForConcurrentCoordinators], coordinators, config.nbRequests, config.nbPeaks / 2, stats), "client1")
      client1 ! "go"
      val client2 = system.actorOf(Props(classOf[ContinuousOverloadClientForConcurrentCoordinators], coordinators, config.nbRequests, config.nbPeaks - (config.nbPeaks / 2), stats), "client2")
      Future { 
