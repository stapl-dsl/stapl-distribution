  ...
  def main(args: Array[String]) {
    ...
    selection.resolveOne(3.seconds).onComplete {
      case Success(coordinator) =>
        ...
        val clients = system.actorOf(Props(classOf[InitialPeakClient],coordinator, nbRequests), "clients")
        clients ! "go"
        println(s"InitialPeak client started at $hostname:$port with $nbRequests requests")
      case Failure(t) =>
        t.printStackTrace()
        system.shutdown
    }
